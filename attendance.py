...
# COOLDOWN_TIME = 10      # seconds between IN/OUT
...
#     # cooldown check
#     if student["last_action_time"] is not None:
#         diff = (now - student["last_action_time"]).total_seconds()
...
#         student["status"] = "IN"
#         student["last_action_time"] = now
#     else:
#         entry_time = student["last_entry"]
#         exit_time = now
#         duration = (exit_time - entry_time).total_seconds()
#         student["total_time"] += duration
#         student["sessions"].append((entry_time, exit_time))
#         student["status"] = "OUT"
#         student["last_action_time"] = now

# # ==============================
# # CAMERA START (Phone WiFi)
# # ==============================
# phone_ip = "192.168.1.27:4747"  # change with your phone IP
...
#         color = (0,255,0) if name!="OUTSIDER STUDENT" else (0,0,255)
#         cv2.rectangle(img,(x1,y1),(x2,y2),color,2)
#         cv2.putText(img,name,(x1,y1-10),cv2.FONT_HERSHEY_SIMPLEX,0.8,color,2)

#         if name != "OUTSIDER STUDENT":
...
#         entry_time = session[0].strftime("%H:%M:%S")
#         exit_time = session[1].strftime("%H:%M:%S")
#         total_minutes = round((session[1]-session[0]).total_seconds()/60,2)
#         status = "PRESENT" if total_minutes >= 40 else "ABSENT"
#         report_data.append({
#             "Name": name,
#             "Date": data["date"],
#             "Day": data["day"],
...
# # Column order fix
# columns_order = ["Name","Date","Day","Entry_Time","Exit_Time","Total_Minutes","Status"]
# df = df[columns_order]

# # Save CSV with PermissionError handling
# file_name = "attendance.csv"
# try:
#     df.to_csv(file_name, mode='w', header=True, index=False)
# except PermissionError:
#     file_name = "attendance_new.csv"
#     df.to_csv(file_name, mode='w', header=True, index=False)
#     print("Original file open. Saved to attendance_new.csv instead.")
...
# # ==============================
# # SETTINGS
# PERIOD_DURATION = 50*60        # 50 minutes
# STRICT_THRESHOLD = 0.55
# COOLDOWN_TIME = 15             # seconds to prevent repeated IN/OUT
# FRAME_SKIP = 5                 # process every 5th frame
...
#     # cooldown to prevent repeated in/out
#     if student["last_action_time"] is not None:
#         diff = (now - student["last_action_time"]).total_seconds()
#         if diff < COOLDOWN_TIME:
#             return
...
#         student["status"] = "OUT"
#         student["last_action_time"] = now
#         # Update exit time only once (or last time in period)
#         student["exit_time"] = now.strftime("%H:%M:%S")

# # ==============================
# # CAMERA START
# phone_ip = "192.168.1.27:4747"   # change to your phone IP
# cap = cv2.VideoCapture(0)
...
# report_data = []
# for name,data in attendance_data.items():
#     # If student is still IN at period end, add remaining time
#     if data["status"]=="IN" and data["last_entry"] is not None:
#         data["total_time"] += (datetime.now() - data["last_entry"]).total_seconds()
#         data["exit_time"] = datetime.now().strftime("%H:%M:%S")
...
#         "Date": data["date"],
#         "Day": data["day"],
#         "Entry_Time": data["entry_time"] if data["entry_time"] else "",
#         "Exit_Time": data["exit_time"] if data["exit_time"] else "",
#         "Total_Minutes": total_minutes,
#         "Status": status
#     })
#     print(f"{name} -> {total_minutes} minutes -> {status}")
...
# FRAME_SKIP = 3
# EXIT_DELAY = 3   # seconds after face disappears
